    ...
    <div className="trio-page relative min-h-full space-y-6 pb-24 sm:pb-8">
      ...
      <header className="trio-aurora trio-shine trio-enter relative overflow-hidden rounded-[32px] border border-white/[.09] p-7 sm:p-10">
        <div className="relative z-10 flex flex-col justify-between gap-8 sm:flex-row sm:items-end">
          <div><p className="trio-kicker">TRIO · PANORAMA FINANCEIRO</p><h1 className="trio-title mt-4 max-w-2xl text-4xl leading-[.92] text-[#f5f5f3] sm:text-6xl">Tudo em <span className="text-brand-400">perspectiva.</span></h1><p className="mt-5 max-w-lg text-base leading-relaxed text-[#b8b8b5]">Um retrato claro das suas finanças em <span className="font-semibold text-[#f5f5f3] capitalize">{new Date().toLocaleDateString("pt-BR", { month: "long", year: "numeric" })}</span>.</p><div className="mt-7 flex flex-wrap gap-2 text-xs text-[#b8b8b5]"><span className="rounded-full border border-white/[.14] bg-black/10 px-3 py-1.5">Atualizado agora</span><span className="rounded-full border border-white/[.14] bg-black/10 px-3 py-1.5">Perfil {currentProfile === "business" ? "Negócio" : "Pessoal"}</span></div></div>
